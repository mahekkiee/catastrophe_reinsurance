...
def create_sample_data():
    """Create sample IRDAI data if file not found"""
    data = {
...
def markov_data():
    """API endpoint for Markov model data"""
    try:
        df = load_data()
        P, pi, df_regime = fit_markov_model(df)
...
def trigger_analysis():
    """API endpoint for trigger analysis"""
    try:
...
def company_profiles():
    """API endpoint for insurance company profiles"""
    try:
        companies = {
            'hdfc': {
                'name': 'HDFC Insurance',
...
def health():
    ...
    return jsonify({'status': 'ok', 'message': 'Catastrophic Loss Clustering API is running'})
